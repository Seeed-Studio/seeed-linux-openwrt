...
(function () {
  ...
  function removePath(filename, isdir) {
    var c = confirm('make sure to delete ' + filename + ' ?');
    if (c) {
  ...
  function renamePath(filename) {
    var newname = prompt('rename:', filename);
    if (newname) {
...
function installPath(filename) {
   var c = confirm('are you sure to install' + filename + '？please check whether it work after a few times and opkg update哦?');
    if (c) {
      iwxhr.get('/cgi-bin/luci/admin/system/filebrowser_install',
        {
          filename: concatPath(currentPath, filename),
        },
        function (x, res) {
          if (res.ec === 0) {
            refresh_list(res.data, currentPath);
  ...
  function refresh_list(filenames, path) {
    ...
    if (filenames) {
      for (var i = 0; i < filenames.length; i++) {
 	  ...
 	  var index1=o.filename.lastIndexOf(".");
          var index2=o.filename.length;
          var suffix=o.filename.substring(index1, index2);
          var exestr = '';
          if(suffix=='.ipk'){
           	exestr += '<button class="cbi-button cbi-button-install">Install</button>';
          }
          listHtml += '<tr class="cbi-section-table-row cbi-rowstyle-' + (1 + i%2)
            ...
            + '<td class="cbi-section-table-cell"><button class="cbi-button cbi-button-edit">Rename</button>\
                <button class="cbi-button cbi-button-remove">Delete</button>'+exestr+'</td>'
            + '</tr>';
        }
      }
    }
